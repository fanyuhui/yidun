...
+ (YidunPlugin *)sharedInstance {
    ...
    static YidunPlugin *sharedObject = nil;
    dispatch_once(&onceToken, ^{
        sharedObject = [[YidunPlugin alloc] init];
    });
...
+ (void)registerWithRegistrar:(NSObject<FlutterPluginRegistrar>*)registrar {
  ...
  [registrar addMethodCallDelegate:instance channel:channel];
    ...
    [eventChannel setStreamHandler:[YidunPlugin sharedInstance]];
...
- (FlutterError* _Nullable)onListenWithArguments:(id _Nullable)arguments
      ...
      if (events) {
          [YidunPlugin sharedInstance].eventSink = events;
      }
...
- (void)verifyCodeInitFailed:(NSString *)message{
    ...
    if ([YidunPlugin sharedInstance].eventSink) {
        [YidunPlugin sharedInstance].eventSink(message);
    }
}
...
- (void)verifyCodeValidateFinish:(BOOL)result validate:(NSString *)validate message:(NSString *)message{
    ...
    if (result) {
        if ([YidunPlugin sharedInstance].eventSink) {
            [YidunPlugin sharedInstance].eventSink(dict);
        }
    } else {
        if ([YidunPlugin sharedInstance].eventSink) {
            [YidunPlugin sharedInstance].eventSink(dict);
...
- (void)verifyCodeCloseWindow:(NTESVerifyCodeClose)close {
    if ([YidunPlugin sharedInstance].eventSink) {
        [YidunPlugin sharedInstance].eventSink([NSNumber numberWithInt:(int)close]);
    }
}
@end
